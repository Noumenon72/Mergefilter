 ...
 *
 */
public class PrefixMerger {

    public final static Logger logger = LogManager.getLogger(PrefixMerger.class.getName());
    ...
    public List<String> mergePrefixes(List<String> sources, List<String> prefixes) {
        ...
        int size = Math.max(sources.size(), prefixes.size());
        mSourceIndex = 0;
        mPrefixIndex = 0;
        mResults = new ArrayList<String>(size);
        mSources = new ArrayList<String>(sources);
        mPrefixes = new LinkedList<String>();
        ...
        ArrayList<String> matchingSources = new ArrayList<>(size);
        List<String> nonmatchingPrefixes = new LinkedList<String>();

        // special cases to save time
        if (sources.size() == 0)
            return prefixes;
        if (prefixes.size() == 0)
            return new ArrayList<String>();
        if (prefixes.get(0).equals("")) {
            mergeEmptyStringPrefixes(sources, matchingSources, nonmatchingPrefixes);
        }

        nonmatchingPrefixes.addAll(mPrefixes);
        int sourcesPosition = 0;
        mPrefixesPosition = 0;
        for (int j = sourcesPosition; j < sources.size(); j++) {
            List<String> matchingPrefixes = startsWithPrefixes(sources.get(j));
            logger.debug("{} starts with these prefixes: {}", sources.get(j), matchingPrefixes);
            if (matchingPrefixes.size() > 0) {
                matchingSources.add(sources.get(j));
                logger.debug("just added {} because matchingprefixes was {}", sources.get(j), matchingPrefixes);
                sourcesPosition = j + 1;
                nonmatchingPrefixes.removeAll(matchingPrefixes);
                logger.debug("Nonmatching prefixes is now {}", nonmatchingPrefixes);
                if (matchingPrefixes.size() == 1) {

                }
            }
        }
        mPrefixes.removeAll(nonmatchingPrefixes);
        List<String> results = collate(matchingSources, nonmatchingPrefixes);
    ...
    private List<String> collate(ArrayList<String> matchingSources, List<String> nonmatchingPrefixes) {
        List<String> results = CollectionUtils.collate(matchingSources, nonmatchingPrefixes,
                String.CASE_INSENSITIVE_ORDER);
        return results;
    }

    private void mergeEmptyStringPrefixes(List<String> sources, ArrayList<String> matchingSources,
            List<String> nonmatchingPrefixes) {
        matchingSources.addAll(sources);
        int sourceIndex = 0;
        Iterator<String> prefixItr = mPrefixes.iterator();
        Iterator<String> sourceItr = sources.iterator();
        while (prefixItr.hasNext()) {
            String prefix = prefixItr.next();
            String source = sourceItr.next();
            boolean addPrefixAtEnd = false;
            while (sourceItr.hasNext()) {
                addPrefixAtEnd = false;
                while (source.compareToIgnoreCase(prefix) < 0 && sourceItr.hasNext()) {
                    source = sourceItr.next();
                }
                while (source.toLowerCase().startsWith(prefix.toLowerCase()) && prefixItr.hasNext()) {
                    prefix = prefixItr.next();
                }

                while (source.compareToIgnoreCase(prefix) > 0) {
                    nonmatchingPrefixes.add(prefix);
                    logger.warn("Added prefix {} after comparing it to source {}", prefix, source);
                    if (prefixItr.hasNext()) {
                        prefix = prefixItr.next();
                    } else {
                        //    logger.warn("Adding prefix {} because it's last in the list", prefix);
                        //                            nonmatchingPrefixes.add(prefix);
                        addPrefixAtEnd = true;
                        break;
                    }
                }
                //can't tell if last prefix advanced to was in
                if (addPrefixAtEnd) {
                    nonmatchingPrefixes.add(prefix);
                } else {
                    prefixItr.forEachRemaining(pref -> {
                        nonmatchingPrefixes.add(pref);
                        logger.warn("Added prefix {} since there are no more sources that could match it", pref);
                    });
                }
                List<String> results = CollectionUtils.collate(matchingSources, nonmatchingPrefixes,
                        String.CASE_INSENSITIVE_ORDER);
    ...
    /**
     * @param source
     *            Target to compare to prefixes
     * @return List of prefixes that matched.
     */
    private List<String> startsWithPrefixes(String source) {
        String sourceLC = source.toLowerCase();
        // if (hadPrefixRun)
        // if source !startsWith mStartOfLastPrefixRun
        // mPrefixIndex += sizeOfLastPrefixRun
        List<String> matchingPrefixes = new ArrayList<>();
        boolean foundMatch = false;
        for (ListIterator<String> itr = mPrefixes.listIterator(mPrefixIndex); itr.hasNext();) {
            String prefix = itr.next();
            // boolean startedSkipping = false;
            // String firstPrefix = prefix;
            // String lastSkippedPrefix = "";
            // while (prefix.compareToIgnoreCase(source) < 0 && itr.hasNext()) {
            // if (!startedSkipping) {
            // startedSkipping = true;
            // }
            // lastSkippedPrefix = prefix;
            // prefix = itr.next();
            // }
            // if (startedSkipping) {
            // logger.debug("Checking source {}, skipped all the prefixes from {} to {}", source, firstPrefix,
            // lastSkippedPrefix);
            // }

            if (sourceLC.startsWith(prefix.toLowerCase())) {
                matchingPrefixes.add(prefix);
                logger.debug("Source {} starts with prefix {}", source, prefix);
                foundMatch = true;
            } else if (source.compareToIgnoreCase(prefix) < 0) {
                logger.debug("Prefix {} comes after source {}, so we can stop looking", prefix, source);
                break; // since they're sorted, once we found the first match, the first nonmatch is the end.
            }
